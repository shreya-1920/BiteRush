export const menuData = {
  1: [
    {
      id: 101,
      name: "Classic Cheeseburger",
      price: 299,
      oldPrice: 349,
      rating: 4.8,
      reviews: 320,
      time: "20 mins",
      veg: false,
      bestseller: true,
      description:
        "Juicy grilled beef patty with cheddar cheese, lettuce, tomato and our signature sauce.",
      image:
        "https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=600&auto=format&fit=crop&q=60",
    },

    {
      id: 102,
      name: "BBQ Chicken Burger",
      price: 349,
      oldPrice: 399,
      rating: 4.9,
      reviews: 286,
      time: "22 mins",
      veg: false,
      bestseller: true,
      description:
        "Grilled chicken breast with smoky BBQ sauce and crispy onions.",
      image:
        "https://images.unsplash.com/photo-1550547660-d9450f859349?w=600&auto=format&fit=crop&q=60",
    },

    {
      id: 103,
      name: "Loaded Cheese Fries",
      price: 199,
      oldPrice: 249,
      rating: 4.7,
      reviews: 198,
      time: "15 mins",
      veg: true,
      bestseller: false,
      description:
        "Golden fries topped with creamy cheese sauce and herbs.",
      image:
        "https://images.unsplash.com/photo-1576107232684-1279f390859f?w=600&auto=format&fit=crop&q=60",
    },

    {
      id: 104,
      name: "Crispy Chicken Wings",
      price: 329,
      oldPrice: 379,
      rating: 4.8,
      reviews: 241,
      time: "25 mins",
      veg: false,
      bestseller: true,
      description:
        "Crunchy chicken wings tossed in spicy peri-peri seasoning.",
      image:
        "https://images.unsplash.com/photo-1527477396000-e27163b481c2?w=600&auto=format&fit=crop&q=60",
    },

    {
      id: 105,
      name: "Veggie Supreme Burger",
      price: 249,
      oldPrice: 299,
      rating: 4.6,
      reviews: 165,
      time: "18 mins",
      veg: true,
      bestseller: false,
      description:
        "Grilled veggie patty with fresh lettuce, tomato and cheese.",
      image:
        "https://images.unsplash.com/photo-1520072959219-c595dc870360?w=600&auto=format&fit=crop&q=60",
    },

    {
      id: 106,
      name: "Chocolate Brownie",
      price: 179,
      oldPrice: 219,
      rating: 4.9,
      reviews: 144,
      time: "10 mins",
      veg: true,
      bestseller: false,
      description:
        "Warm chocolate brownie served with rich chocolate drizzle.",
      image:
        "https://images.unsplash.com/photo-1606313564200-e75d5e30476c?w=600&auto=format&fit=crop&q=60",
    },

    {
      id:107,
      name: "Cold Coffee",
      price: 149,
      oldPrice: 179,
      rating: 4.7,
      reviews: 132,
      time: "8 mins",
      veg: true,
      bestseller: false,
      description:
        "Creamy cold coffee blended with ice cream and chocolate.",
      image:
        "https://images.unsplash.com/photo-1461023058943-07fcbe16d735?w=600&auto=format&fit=crop&q=60",
    },

    {
      id: 108,
      name: "Oreo Milkshake",
      price: 199,
      oldPrice: 239,
      rating: 4.8,
      reviews: 189,
      time: "8 mins",
      veg: true,
      bestseller: true,
      description:
        "Rich Oreo milkshake topped with whipped cream and cookie crumbs.",
      image:
        "https://images.unsplash.com/photo-1572490122747-3968b75cc699?w=600&auto=format&fit=crop&q=60",
    },
  ],

  2:[
{
id:201,
name:"Margherita Pizza",
price:399,
oldPrice:459,
rating:4.8,
reviews:295,
time:"25 mins",
veg:true,
bestseller:true,
description:"Classic wood-fired pizza topped with mozzarella, basil and tomato sauce.",
image:"https://images.unsplash.com/photo-1574071318508-1cdbab80d002?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NHx8TWFyZ2hlcml0YSUyMFBpenphfGVufDB8fDB8fHww"
},
{
id:202,
name:"Pepperoni Pizza",
price:499,
oldPrice:549,
rating:4.9,
reviews:310,
time:"28 mins",
veg:false,
bestseller:true,
description:"Loaded pepperoni with melted mozzarella cheese.",
image:"https://images.unsplash.com/photo-1628840042765-356cda07504e?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NHx8UGVwcGVyb25pJTIwUGl6emF8ZW58MHx8MHx8fDA%3D"
},
{
id:203,
name:"Creamy Alfredo Pasta",
price:349,
oldPrice:399,
rating:4.7,
reviews:180,
time:"22 mins",
veg:true,
bestseller:false,
description:"Penne pasta tossed in rich Alfredo sauce.",
image:"https://images.unsplash.com/photo-1662197480393-2a82030b7b83?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8Q3JlYW15JTIwQWxmcmVkbyUyMFBhc3RhfGVufDB8fDB8fHww"
},
{
id:204,
name:"Garlic Bread",
price:179,
oldPrice:229,
rating:4.6,
reviews:120,
time:"12 mins",
veg:true,
bestseller:false,
description:"Freshly baked garlic bread with herbs.",
image:"https://images.unsplash.com/photo-1593527270723-834c53a3fed4?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8OHx8Z2FybGljJTIwYnJlYWR8ZW58MHx8MHx8fDA%3D"
},
{
id:205,
name:"Lasagna",
price:429,
oldPrice:489,
rating:4.8,
reviews:190,
time:"30 mins",
veg:false,
bestseller:true,
description:"Layers of pasta with cheese and meat sauce.",
image:"https://plus.unsplash.com/premium_photo-1671559021023-3da68c12aeed?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NXx8TGFzYWduYXxlbnwwfHwwfHx8MA%3D%3D"
},
{
id:206,
name:"Tiramisu",
price:249,
oldPrice:299,
rating:4.9,
reviews:165,
time:"10 mins",
veg:true,
bestseller:false,
description:"Classic Italian coffee-flavoured dessert.",
image:"https://plus.unsplash.com/premium_photo-1695028378225-97fbe39df62a?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8dGlyYW1pc3V8ZW58MHx8MHx8fDA%3D"
},
{
id:207,
name:"Italian Lemonade",
price:149,
oldPrice:179,
rating:4.7,
reviews:88,
time:"5 mins",
veg:true,
bestseller:false,
description:"Refreshing sparkling lemonade.",
image:"https://images.unsplash.com/photo-1623084921164-4a8c5c37a912?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8M3x8aXRhbGlhbiUyMGxlbW9uYWRlfGVufDB8fDB8fHww"
}
],
  3: [
  {
    id: 301,
    name: "Signature Sushi Platter",
    price: 599,
    oldPrice: 699,
    rating: 4.9,
    reviews: 412,
    time: "25 mins",
    veg: false,
    bestseller: true,
    description:
      "Fresh assortment of salmon, tuna and California rolls served with soy sauce and wasabi.",
    image:
      "https://images.unsplash.com/photo-1579871494447-9811cf80d66c?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 302,
    name: "Chicken Ramen",
    price: 429,
    oldPrice: 479,
    rating: 4.8,
    reviews: 298,
    time: "22 mins",
    veg: false,
    bestseller: true,
    description:
      "Rich chicken broth with noodles, soft-boiled egg and grilled chicken slices.",
    image:
      "https://images.unsplash.com/photo-1617093727343-374698b1b08d?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 303,
    name: "Veg Tempura",
    price: 299,
    oldPrice: 349,
    rating: 4.7,
    reviews: 186,
    time: "18 mins",
    veg: true,
    bestseller: false,
    description:
      "Crispy battered vegetables served with authentic tempura dipping sauce.",
    image:
      "https://images.unsplash.com/photo-1607301405390-d831c242f59b?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 304,
    name: "Teriyaki Chicken",
    price: 499,
    oldPrice: 549,
    rating: 4.8,
    reviews: 243,
    time: "24 mins",
    veg: false,
    bestseller: true,
    description:
      "Grilled chicken glazed with homemade teriyaki sauce and steamed rice.",
    image:
      "https://images.unsplash.com/photo-1512058564366-18510be2db19?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 305,
    name: "Salmon Nigiri",
    price: 449,
    oldPrice: 499,
    rating: 4.9,
    reviews: 172,
    time: "20 mins",
    veg: false,
    bestseller: false,
    description:
      "Fresh salmon served over seasoned sushi rice with traditional garnish.",
    image:
      "https://images.unsplash.com/photo-1553621042-f6e147245754?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 306,
    name: "Miso Soup",
    price: 179,
    oldPrice: 219,
    rating: 4.6,
    reviews: 120,
    time: "10 mins",
    veg: true,
    bestseller: false,
    description:
      "Traditional Japanese soup with tofu, seaweed and spring onions.",
    image:
      "https://images.unsplash.com/photo-1547592180-85f173990554?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 307,
    name: "Matcha Cheesecake",
    price: 249,
    oldPrice: 299,
    rating: 4.8,
    reviews: 154,
    time: "8 mins",
    veg: true,
    bestseller: false,
    description:
      "Creamy Japanese-style cheesecake infused with premium matcha.",
    image:
      "https://images.unsplash.com/photo-1565958011703-44f9829ba187?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 308,
    name: "Japanese Green Tea",
    price: 129,
    oldPrice: 159,
    rating: 4.7,
    reviews: 96,
    time: "5 mins",
    veg: true,
    bestseller: false,
    description:
      "Refreshing authentic Japanese green tea served hot.",
    image:
      "https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?w=600&auto=format&fit=crop&q=60",
  },
],
  4: [
  {
    id: 401,
    name: "Butter Chicken",
    price: 429,
    oldPrice: 499,
    rating: 4.9,
    reviews: 458,
    time: "28 mins",
    veg: false,
    bestseller: true,
    description:
      "Tender chicken cooked in rich buttery tomato gravy with aromatic Indian spices.",
    image:
      "https://images.unsplash.com/photo-1603894584373-5ac82b2ae398?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 402,
    name: "Paneer Butter Masala",
    price: 379,
    oldPrice: 429,
    rating: 4.8,
    reviews: 356,
    time: "25 mins",
    veg: true,
    bestseller: true,
    description:
      "Soft paneer cubes simmered in creamy tomato gravy with butter and spices.",
    image:
      "https://images.unsplash.com/photo-1631452180519-c014fe946bc7?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 403,
    name: "Hyderabadi Biryani",
    price: 449,
    oldPrice: 519,
    rating: 4.9,
    reviews: 390,
    time: "30 mins",
    veg: false,
    bestseller: true,
    description:
      "Fragrant basmati rice layered with tender chicken and traditional spices.",
    image:
      "https://images.unsplash.com/photo-1563379091339-03246963d29a?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 404,
    name: "Garlic Butter Naan",
    price: 99,
    oldPrice: 129,
    rating: 4.7,
    reviews: 210,
    time: "12 mins",
    veg: true,
    bestseller: false,
    description:
      "Freshly baked naan brushed with butter, garlic and coriander.",
    image:
      "https://images.unsplash.com/photo-1610057099443-fde8c4d50f91?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 405,
    name: "Dal Makhani",
    price: 289,
    oldPrice: 339,
    rating: 4.8,
    reviews: 198,
    time: "20 mins",
    veg: true,
    bestseller: false,
    description:
      "Slow-cooked black lentils finished with butter and fresh cream.",
    image:
      "https://images.unsplash.com/photo-1546833999-b9f581a1996d?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 406,
    name: "Tandoori Chicken",
    price: 469,
    oldPrice: 539,
    rating: 4.8,
    reviews: 255,
    time: "28 mins",
    veg: false,
    bestseller: true,
    description:
      "Char-grilled chicken marinated overnight with yogurt and Indian spices.",
    image:
      "https://images.unsplash.com/photo-1599487488170-d11ec9c172f0?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 407,
    name: "Gulab Jamun",
    price: 149,
    oldPrice: 179,
    rating: 4.9,
    reviews: 180,
    time: "8 mins",
    veg: true,
    bestseller: false,
    description:
      "Soft milk dumplings soaked in warm cardamom-flavoured sugar syrup.",
    image:
      "https://images.unsplash.com/photo-1601050690597-df0568f70950?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 408,
    name: "Mango Lassi",
    price: 159,
    oldPrice: 199,
    rating: 4.8,
    reviews: 145,
    time: "5 mins",
    veg: true,
    bestseller: false,
    description:
      "Refreshing yogurt drink blended with sweet Alphonso mangoes.",
    image:
      "https://images.unsplash.com/photo-1623065422902-30a2d299bbe4?w=600&auto=format&fit=crop&q=60",
  },
],
  5: [
  {
    id: 501,
    name: "Hakka Noodles",
    price: 299,
    oldPrice: 349,
    rating: 4.8,
    reviews: 321,
    time: "18 mins",
    veg: true,
    bestseller: true,
    description:
      "Classic stir-fried Hakka noodles tossed with fresh vegetables and Asian sauces.",
    image:
      "https://images.unsplash.com/photo-1617622141675-d3005b9067c5?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 502,
    name: "Schezwan Fried Rice",
    price: 329,
    oldPrice: 379,
    rating: 4.7,
    reviews: 285,
    time: "20 mins",
    veg: true,
    bestseller: true,
    description:
      "Spicy Schezwan-style fried rice loaded with vegetables and aromatic spices.",
    image:
      "https://images.unsplash.com/photo-1603133872878-684f208fb84b?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 503,
    name: "Veg Manchurian",
    price: 289,
    oldPrice: 339,
    rating: 4.8,
    reviews: 246,
    time: "18 mins",
    veg: true,
    bestseller: false,
    description:
      "Crispy vegetable balls tossed in a spicy Indo-Chinese Manchurian sauce.",
    image:
      "https://images.unsplash.com/photo-1633945274405-b6c8069047b0?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 504,
    name: "Chicken Dumplings",
    price: 359,
    oldPrice: 419,
    rating: 4.9,
    reviews: 302,
    time: "22 mins",
    veg: false,
    bestseller: true,
    description:
      "Steamed chicken dumplings served with spicy soy dipping sauce.",
    image:
      "https://images.unsplash.com/photo-1563245372-f21724e3856d?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 505,
    name: "Spring Rolls",
    price: 199,
    oldPrice: 239,
    rating: 4.6,
    reviews: 178,
    time: "15 mins",
    veg: true,
    bestseller: false,
    description:
      "Golden crispy spring rolls stuffed with fresh vegetables.",
    image:
      "https://images.unsplash.com/photo-1606491956689-2ea866880c84?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 506,
    name: "Hot & Sour Soup",
    price: 179,
    oldPrice: 219,
    rating: 4.7,
    reviews: 154,
    time: "12 mins",
    veg: true,
    bestseller: false,
    description:
      "Authentic Chinese soup with vegetables, mushrooms and black pepper.",
    image:
      "https://images.unsplash.com/photo-1547592180-85f173990554?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 507,
    name: "Honey Chilli Potato",
    price: 249,
    oldPrice: 299,
    rating: 4.8,
    reviews: 229,
    time: "16 mins",
    veg: true,
    bestseller: true,
    description:
      "Crispy potato fingers tossed in sweet and spicy honey chilli sauce.",
    image:
      "https://images.unsplash.com/photo-1512058564366-18510be2db19?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 508,
    name: "Bubble Tea",
    price: 199,
    oldPrice: 239,
    rating: 4.7,
    reviews: 141,
    time: "8 mins",
    veg: true,
    bestseller: false,
    description:
      "Refreshing milk tea served with chewy tapioca pearls.",
    image:
      "https://images.unsplash.com/photo-1558857563-c0c2c9d79d28?w=600&auto=format&fit=crop&q=60",
  },
],
  6: [
  {
    id: 601,
    name: "Chicken Street Tacos",
    price: 299,
    oldPrice: 349,
    rating: 4.9,
    reviews: 342,
    time: "18 mins",
    veg: false,
    bestseller: true,
    description:
      "Soft corn tortillas filled with grilled chicken, salsa fresca and cilantro.",
    image:
      "https://images.unsplash.com/photo-1552332386-f8dd00dc2f85?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 602,
    name: "Veggie Fiesta Taco",
    price: 249,
    oldPrice: 299,
    rating: 4.7,
    reviews: 198,
    time: "16 mins",
    veg: true,
    bestseller: false,
    description:
      "Loaded with grilled vegetables, beans, cheese and fresh guacamole.",
    image:
      "https://images.unsplash.com/photo-1613514785940-daed07799d9b?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 603,
    name: "Loaded Chicken Burrito",
    price: 389,
    oldPrice: 449,
    rating: 4.8,
    reviews: 287,
    time: "22 mins",
    veg: false,
    bestseller: true,
    description:
      "Large tortilla stuffed with chicken, rice, beans, cheese and chipotle sauce.",
    image:
      "https://images.unsplash.com/photo-1626700051175-6818013e1d4f?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 604,
    name: "Cheesy Quesadilla",
    price: 329,
    oldPrice: 379,
    rating: 4.8,
    reviews: 221,
    time: "20 mins",
    veg: true,
    bestseller: true,
    description:
      "Grilled tortilla filled with mozzarella, cheddar and smoky Mexican spices.",
    image:
      "https://images.unsplash.com/photo-1618040996337-56904b7850b9?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 605,
    name: "Nachos Supreme",
    price: 279,
    oldPrice: 329,
    rating: 4.7,
    reviews: 204,
    time: "15 mins",
    veg: true,
    bestseller: false,
    description:
      "Crunchy tortilla chips topped with cheese sauce, salsa, jalapeños and sour cream.",
    image:
      "https://images.unsplash.com/photo-1513456852971-30c0b8199d4d?w=600&auto=format&fit=crop&q=60",
  },

  {
    id: 606,
    name: "Mexican Rice Bowl",
    price: 349,
    oldPrice: 399,
    rating: 4.8,
    reviews: 176,
    time: "20 mins",
    veg: true,
    bestseller: false,
    description:
      "Mexican rice served with beans, corn, salsa and creamy avocado dressing.",
    image:
      "https://plus.unsplash.com/premium_photo-1671485196355-32005a27fd02?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8bWV4aWNhbiUyMHJpY2UlMjBib3dsfGVufDB8fDB8fHww",
  },

  {
    id: 607,
    name: "Churros with Chocolate",
    price: 199,
    oldPrice: 239,
    rating: 4.9,
    reviews: 152,
    time: "10 mins",
    veg: true,
    bestseller: false,
    description:
      "Fresh cinnamon churros served with warm chocolate dipping sauce.",
    image:
      "https://plus.unsplash.com/premium_photo-1714180194796-8cb272b0bfed?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8Y2h1cnJvcyUyMHdpdGglMjBjaG9jb2xhdGV8ZW58MHx8MHx8fDA%3D",
  },

  {
    id: 608,
    name: "Classic Lemon Soda",
    price: 149,
    oldPrice: 179,
    rating: 4.6,
    reviews: 118,
    time: "5 mins",
    veg: true,
    bestseller: false,
    description:
      "Fresh sparkling lemon soda with mint and crushed ice.",
    image:
      "https://images.unsplash.com/photo-1513558161293-cdaf765ed2fd?w=600&auto=format&fit=crop&q=60",
  },
],
  7:[ 
  {
    id: 701,
    name: "Grilled Salmon",
    price: 499,
    oldPrice: 559,
    rating: 4.8,
    reviews: 356,
    time: "20 mins",
    category: "Main",
    isVeg: false,
    bestseller: true,
    description: "Perfectly grilled Atlantic salmon served with lemon butter sauce.",
    image: "https://images.unsplash.com/photo-1467003909585-2f8a72700288?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 702,
    name: "Garlic Butter Prawns",
    price: 459,
    oldPrice: 519,
    rating: 4.7,
    reviews: 284,
    time: "18 mins",
    category: "Main",
    isVeg: false,
    bestseller: true,
    description: "Juicy prawns tossed in rich garlic butter and herbs.",
    image: "https://images.unsplash.com/photo-1563379091339-03246963d29d?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 703,
    name: "Fish & Chips",
    price: 389,
    oldPrice: 439,
    rating: 4.6,
    reviews: 215,
    time: "22 mins",
    category: "Main",
    isVeg: false,
    bestseller: false,
    description: "Golden fried fish fillet served with crispy fries and tartar sauce.",
    image: "https://images.unsplash.com/photo-1544025162-d76694265947?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 704,
    name: "Seafood Platter",
    price: 699,
    oldPrice: 799,
    rating: 4.9,
    reviews: 432,
    time: "30 mins",
    category: "Special",
    isVeg: false,
    bestseller: true,
    description: "A premium platter of prawns, fish, calamari and grilled seafood.",
    image: "https://images.unsplash.com/photo-1559847844-5315695dadae?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 705,
    name: "Lemon Herb Fish",
    price: 429,
    oldPrice: 489,
    rating: 4.7,
    reviews: 176,
    time: "19 mins",
    category: "Main",
    isVeg: false,
    bestseller: false,
    description: "Pan-seared fish seasoned with fresh herbs and lemon butter.",
    image: "https://images.unsplash.com/photo-1519708227418-c8fd9a32b7a2?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 706,
    name: "Crispy Calamari",
    price: 349,
    oldPrice: 399,
    rating: 4.6,
    reviews: 192,
    time: "15 mins",
    category: "Starter",
    isVeg: false,
    bestseller: false,
    description: "Lightly battered calamari served with spicy aioli dip.",
    image: "https://images.unsplash.com/photo-1625944230945-1b7dd3b949ab?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 707,
    name: "Shrimp Fried Rice",
    price: 329,
    oldPrice: 379,
    rating: 4.5,
    reviews: 168,
    time: "16 mins",
    category: "Rice",
    isVeg: false,
    bestseller: false,
    description: "Flavorful fried rice tossed with shrimp, vegetables and soy sauce.",
    image: "https://images.unsplash.com/photo-1512058564366-18510be2db19?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 708,
    name: "Lobster Bisque",
    price: 299,
    oldPrice: 349,
    rating: 4.7,
    reviews: 138,
    time: "12 mins",
    category: "Soup",
    isVeg: false,
    bestseller: false,
    description: "Creamy French-style lobster soup with rich seafood flavors.",
    image: "https://images.unsplash.com/photo-1547592166-23ac45744acd?w=600&auto=format&fit=crop&q=60"
  }
],
  8:  [
 {
    id: 701,
    name: "Grilled Salmon",
    price: 499,
    oldPrice: 559,
    rating: 4.8,
    reviews: 356,
    time: "20 mins",
    category: "Main",
    isVeg: false,
    bestseller: true,
    description: "Perfectly grilled Atlantic salmon served with lemon butter sauce.",
    image: "https://images.unsplash.com/photo-1467003909585-2f8a72700288?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 702,
    name: "Garlic Butter Prawns",
    price: 459,
    oldPrice: 519,
    rating: 4.7,
    reviews: 284,
    time: "18 mins",
    category: "Main",
    isVeg: false,
    bestseller: true,
    description: "Juicy prawns tossed in rich garlic butter and herbs.",
    image: "https://images.unsplash.com/photo-1563379091339-03246963d29d?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 703,
    name: "Fish & Chips",
    price: 389,
    oldPrice: 439,
    rating: 4.6,
    reviews: 215,
    time: "22 mins",
    category: "Main",
    isVeg: false,
    bestseller: false,
    description: "Golden fried fish fillet served with crispy fries and tartar sauce.",
    image: "https://images.unsplash.com/photo-1544025162-d76694265947?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 704,
    name: "Seafood Platter",
    price: 699,
    oldPrice: 799,
    rating: 4.9,
    reviews: 432,
    time: "30 mins",
    category: "Special",
    isVeg: false,
    bestseller: true,
    description: "A premium platter of prawns, fish, calamari and grilled seafood.",
    image: "https://images.unsplash.com/photo-1559847844-5315695dadae?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 705,
    name: "Lemon Herb Fish",
    price: 429,
    oldPrice: 489,
    rating: 4.7,
    reviews: 176,
    time: "19 mins",
    category: "Main",
    isVeg: false,
    bestseller: false,
    description: "Pan-seared fish seasoned with fresh herbs and lemon butter.",
    image: "https://images.unsplash.com/photo-1519708227418-c8fd9a32b7a2?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 706,
    name: "Crispy Calamari",
    price: 349,
    oldPrice: 399,
    rating: 4.6,
    reviews: 192,
    time: "15 mins",
    category: "Starter",
    isVeg: false,
    bestseller: false,
    description: "Lightly battered calamari served with spicy aioli dip.",
    image: "https://images.unsplash.com/photo-1625944230945-1b7dd3b949ab?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 707,
    name: "Shrimp Fried Rice",
    price: 329,
    oldPrice: 379,
    rating: 4.5,
    reviews: 168,
    time: "16 mins",
    category: "Rice",
    isVeg: false,
    bestseller: false,
    description: "Flavorful fried rice tossed with shrimp, vegetables and soy sauce.",
    image: "https://images.unsplash.com/photo-1512058564366-18510be2db19?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 708,
    name: "Lobster Bisque",
    price: 299,
    oldPrice: 349,
    rating: 4.7,
    reviews: 138,
    time: "12 mins",
    category: "Soup",
    isVeg: false,
    bestseller: false,
    description: "Creamy French-style lobster soup with rich seafood flavors.",
    image: "https://images.unsplash.com/photo-1547592166-23ac45744acd?w=600&auto=format&fit=crop&q=60"
  }
],
9: [
  {
    id: 901,
    name: "Cappuccino",
    price: 169,
    oldPrice: 199,
    rating: 4.8,
    reviews: 542,
    time: "6 mins",
    category: "Coffee",
    isVeg: true,
    bestseller: true,
    description: "Rich espresso topped with velvety steamed milk foam.",
    image: "https://images.unsplash.com/photo-1509042239860-f550ce710b93?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 902,
    name: "Café Latte",
    price: 189,
    oldPrice: 219,
    rating: 4.7,
    reviews: 386,
    time: "7 mins",
    category: "Coffee",
    isVeg: true,
    bestseller: false,
    description: "Smooth espresso blended with creamy steamed milk.",
    image: "https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 903,
    name: "Butter Croissant",
    price: 149,
    oldPrice: 179,
    rating: 4.8,
    reviews: 291,
    time: "5 mins",
    category: "Bakery",
    isVeg: true,
    bestseller: true,
    description: "Freshly baked buttery croissant with a flaky golden crust.",
    image: "https://images.unsplash.com/photo-1635341083259-7a2a471541c1?q=80&w=735&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
  },
  {
    id: 904,
    name: "Blueberry Cheesecake",
    price: 249,
    oldPrice: 289,
    rating: 4.9,
    reviews: 438,
    time: "8 mins",
    category: "Dessert",
    isVeg: true,
    bestseller: true,
    description: "Creamy cheesecake topped with fresh blueberry compote.",
    image: "https://images.unsplash.com/photo-1533134242443-d4fd215305ad?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 905,
    name: "Chocolate Brownie",
    price: 199,
    oldPrice: 229,
    rating: 4.7,
    reviews: 267,
    time: "6 mins",
    category: "Dessert",
    isVeg: true,
    bestseller: false,
    description: "Warm fudgy chocolate brownie served with chocolate drizzle.",
    image: "https://images.unsplash.com/photo-1606313564200-e75d5e30476c?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 906,
    name: "French Toast",
    price: 239,
    oldPrice: 279,
    rating: 4.8,
    reviews: 214,
    time: "12 mins",
    category: "Breakfast",
    isVeg: true,
    bestseller: false,
    description: "Golden French toast topped with maple syrup and berries.",
    image: "https://images.unsplash.com/photo-1484723091739-30a097e8f929?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 907,
    name: "Cold Coffee",
    price: 199,
    oldPrice: 239,
    rating: 4.8,
    reviews: 325,
    time: "5 mins",
    category: "Drinks",
    isVeg: true,
    bestseller: true,
    description: "Creamy chilled coffee blended with ice cream and chocolate.",
    image: "https://images.unsplash.com/photo-1517701604599-bb29b565090c?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 908,
    name: "Hazelnut Mocha",
    price: 229,
    oldPrice: 269,
    rating: 4.9,
    reviews: 301,
    time: "7 mins",
    category: "Coffee",
    isVeg: true,
    bestseller: true,
    description: "Espresso infused with rich chocolate and roasted hazelnut syrup.",
    image: "https://images.unsplash.com/photo-1494314671902-399b18174975?w=600&auto=format&fit=crop&q=60"
  }
],
  10: [
  {
    id: 1001,
    name: "Chicken Dum Biryani",
    price: 359,
    oldPrice: 419,
    rating: 4.9,
    reviews: 762,
    time: "25 mins",
    category: "Biryani",
    isVeg: false,
    bestseller: true,
    description: "Authentic Hyderabadi dum biryani layered with fragrant basmati rice and tender chicken.",
    image: "https://images.unsplash.com/photo-1633945274405-b6c8069047b0?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1002,
    name: "Mutton Biryani",
    price: 429,
    oldPrice: 489,
    rating: 4.8,
    reviews: 534,
    time: "28 mins",
    category: "Biryani",
    isVeg: false,
    bestseller: true,
    description: "Slow-cooked mutton biryani infused with aromatic spices.",
    image: "https://images.unsplash.com/photo-1701579231349-d7459c40919d?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1003,
    name: "Veg Hyderabadi Biryani",
    price: 279,
    oldPrice: 329,
    rating: 4.7,
    reviews: 321,
    time: "22 mins",
    category: "Biryani",
    isVeg: true,
    bestseller: false,
    description: "Flavorful vegetable biryani cooked with saffron and aromatic spices.",
    image: "https://images.unsplash.com/photo-1589302168068-964664d93dc0?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1004,
    name: "Butter Chicken",
    price: 389,
    oldPrice: 449,
    rating: 4.9,
    reviews: 618,
    time: "20 mins",
    category: "Curry",
    isVeg: false,
    bestseller: true,
    description: "Creamy tomato gravy with juicy tandoori chicken pieces.",
    image: "https://images.unsplash.com/photo-1603894584373-5ac82b2ae398?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1005,
    name: "Paneer Butter Masala",
    price: 299,
    oldPrice: 349,
    rating: 4.8,
    reviews: 396,
    time: "18 mins",
    category: "Curry",
    isVeg: true,
    bestseller: true,
    description: "Soft paneer cubes cooked in rich buttery tomato gravy.",
    image: "https://images.unsplash.com/photo-1567188040759-fb8a883dc6d8?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1006,
    name: "Garlic Naan",
    price: 79,
    oldPrice: 99,
    rating: 4.7,
    reviews: 248,
    time: "8 mins",
    category: "Bread",
    isVeg: true,
    bestseller: false,
    description: "Freshly baked tandoor naan topped with garlic butter.",
    image: "https://images.unsplash.com/photo-1671507136750-05ebd0f97843?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1007,
    name: "Chicken Seekh Kebab",
    price: 319,
    oldPrice: 369,
    rating: 4.8,
    reviews: 301,
    time: "15 mins",
    category: "Starter",
    isVeg: false,
    bestseller: false,
    description: "Juicy minced chicken kebabs grilled to smoky perfection.",
    image: "https://images.unsplash.com/photo-1529563021893-cc83c992d75d?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1008,
    name: "Gulab Jamun",
    price: 99,
    oldPrice: 129,
    rating: 4.8,
    reviews: 287,
    time: "5 mins",
    category: "Dessert",
    isVeg: true,
    bestseller: true,
    description: "Soft milk dumplings soaked in warm cardamom sugar syrup.",
    image: "https://images.unsplash.com/photo-1601050690597-df0568f70950?w=600&auto=format&fit=crop&q=60"
  }
],
11: [
  {
    id: 1101,
    name: "Classic Cheeseburger",
    price: 249,
    oldPrice: 299,
    rating: 4.8,
    reviews: 548,
    time: "14 mins",
    category: "Burger",
    isVeg: false,
    bestseller: true,
    description: "Juicy grilled beef patty with cheddar cheese, lettuce and signature sauce.",
    image: "https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1102,
    name: "Double BBQ Burger",
    price: 329,
    oldPrice: 379,
    rating: 4.9,
    reviews: 432,
    time: "16 mins",
    category: "Burger",
    isVeg: false,
    bestseller: true,
    description: "Double grilled patties layered with smoky BBQ sauce and melted cheese.",
    image: "https://images.unsplash.com/photo-1553979459-d2229ba7433b?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1103,
    name: "Crispy Chicken Burger",
    price: 289,
    oldPrice: 339,
    rating: 4.8,
    reviews: 385,
    time: "15 mins",
    category: "Burger",
    isVeg: false,
    bestseller: true,
    description: "Crispy fried chicken fillet with fresh veggies and creamy mayo.",
    image: "https://images.unsplash.com/photo-1550317138-10000687a72b?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1104,
    name: "Veg Supreme Burger",
    price: 219,
    oldPrice: 259,
    rating: 4.7,
    reviews: 274,
    time: "13 mins",
    category: "Burger",
    isVeg: true,
    bestseller: false,
    description: "Loaded vegetable patty with fresh lettuce, tomatoes and cheese.",
    image: "https://images.unsplash.com/photo-1520072959219-c595dc870360?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1105,
    name: "Loaded Fries",
    price: 169,
    oldPrice: 209,
    rating: 4.7,
    reviews: 318,
    time: "10 mins",
    category: "Sides",
    isVeg: true,
    bestseller: true,
    description: "Golden fries loaded with cheese sauce, jalapeños and herbs.",
    image: "https://images.unsplash.com/photo-1576107232684-1279f390859f?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1106,
    name: "Chicken Nuggets",
    price: 229,
    oldPrice: 269,
    rating: 4.8,
    reviews: 291,
    time: "11 mins",
    category: "Sides",
    isVeg: false,
    bestseller: false,
    description: "Crispy chicken nuggets served with spicy dipping sauce.",
    image: "https://images.unsplash.com/photo-1562967916-eb82221dfb92?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1107,
    name: "Coke",
    price: 69,
    oldPrice: 89,
    rating: 4.6,
    reviews: 198,
    time: "2 mins",
    category: "Drinks",
    isVeg: true,
    bestseller: false,
    description: "Chilled Coca-Cola served ice cold.",
    image: "https://images.unsplash.com/photo-1629203851122-3726ecdf080e?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1108,
    name: "Chocolate Shake",
    price: 179,
    oldPrice: 219,
    rating: 4.8,
    reviews: 336,
    time: "6 mins",
    category: "Drinks",
    isVeg: true,
    bestseller: true,
    description: "Rich chocolate milkshake topped with whipped cream.",
    image: "https://images.unsplash.com/photo-1572490122747-3968b75cc699?w=600&auto=format&fit=crop&q=60"
  }
],
  12: [
  {
    id: 1201,
    name: "Creamy Alfredo Pasta",
    price: 329,
    oldPrice: 389,
    rating: 4.9,
    reviews: 468,
    time: "18 mins",
    category: "Pasta",
    isVeg: true,
    bestseller: true,
    description: "Creamy fettuccine pasta tossed in rich Alfredo sauce with parmesan cheese.",
    image: "https://images.unsplash.com/photo-1645112411341-6c4fd023714a?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1202,
    name: "Spaghetti Arrabbiata",
    price: 299,
    oldPrice: 349,
    rating: 4.8,
    reviews: 352,
    time: "17 mins",
    category: "Pasta",
    isVeg: true,
    bestseller: false,
    description: "Classic spaghetti cooked in a spicy garlic tomato Arrabbiata sauce.",
    image: "https://images.unsplash.com/photo-1621996346565-e3dbc646d9a9?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1203,
    name: "Classic Lasagna",
    price: 369,
    oldPrice: 429,
    rating: 4.9,
    reviews: 318,
    time: "22 mins",
    category: "Lasagna",
    isVeg: true,
    bestseller: true,
    description: "Layers of pasta, rich tomato sauce, creamy béchamel and melted cheese.",
    image: "https://images.unsplash.com/photo-1619895092538-128341789043?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1204,
    name: "Margherita Pizza",
    price: 349,
    oldPrice: 399,
    rating: 4.8,
    reviews: 524,
    time: "20 mins",
    category: "Pizza",
    isVeg: true,
    bestseller: true,
    description: "Wood-fired pizza topped with mozzarella, basil and tomato sauce.",
    image: "https://images.unsplash.com/photo-1513104890138-7c749659a591?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1205,
    name: "Garlic Bread",
    price: 149,
    oldPrice: 179,
    rating: 4.7,
    reviews: 284,
    time: "8 mins",
    category: "Sides",
    isVeg: true,
    bestseller: false,
    description: "Freshly baked garlic bread brushed with butter and herbs.",
    image: "https://images.unsplash.com/photo-1573140247632-f8fd74997d5c?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1206,
    name: "Mushroom Risotto",
    price: 359,
    oldPrice: 419,
    rating: 4.8,
    reviews: 241,
    time: "20 mins",
    category: "Rice",
    isVeg: true,
    bestseller: false,
    description: "Creamy Italian risotto cooked with mushrooms, parmesan and herbs.",
    image: "https://images.unsplash.com/photo-1476124369491-e7addf5db371?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1207,
    name: "Tiramisu",
    price: 249,
    oldPrice: 299,
    rating: 4.9,
    reviews: 397,
    time: "6 mins",
    category: "Dessert",
    isVeg: true,
    bestseller: true,
    description: "Classic Italian dessert layered with coffee-soaked ladyfingers and mascarpone cream.",
    image: "https://images.unsplash.com/photo-1571877227200-a0d98ea607e9?w=600&auto=format&fit=crop&q=60"
  },
  {
    id: 1208,
    name: "Italian Lemonade",
    price: 159,
    oldPrice: 199,
    rating: 4.7,
    reviews: 214,
    time: "5 mins",
    category: "Drinks",
    isVeg: true,
    bestseller: false,
    description: "Refreshing sparkling lemonade infused with fresh lemons and mint.",
    image: "https://images.unsplash.com/photo-1513558161293-cdaf765ed2fd?w=600&auto=format&fit=crop&q=60"
  }
]
};